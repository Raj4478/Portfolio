...
export default function Footer({ user }) {
  ...
  return (
    <footer className="relative border-t border-[rgba(99,130,195,0.1)] py-10">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <p className="font-mono text-xs text-[#6b7fa3]">
              Designed & Built by <span className="text-[#a8c3ff] font-medium">{user?.name || "Rajeshwar Singh"}</span>
            </p>
          ...
          <p className="font-mono text-xs text-[#3d4f6b] flex items-center gap-1">
            © {new Date().getFullYear()} · Made with <Heart size={11} className="text-[#f87171]" fill="currentColor" /> in India
          </p>
        </div>
      </div>
    </footer>
