...
function BarCard({ area, icon, skills, level, color, index }) {
  ...
  return (
    <motion.div
      ...
      className="glass rounded-2xl p-4 sm:p-5 neon-border hover:border-[rgba(79,142,247,0.3)] transition-all duration-300"
    >
  ...
  return (
    <section id="skills" className="py-20 sm:py-32 relative">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-14 sm:pt-16">
        ...
        {/* Marquee strip */}
        <div className="relative overflow-hidden py-3 mb-4">
          <div className="absolute left-0 top-0 bottom-0 w-12 sm:w-20 bg-gradient-to-r from-[#050810] to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-12 sm:w-20 bg-gradient-to-l from-[#050810] to-transparent z-10 pointer-events-none" />
          <div className="flex gap-3 marquee whitespace-nowrap">
            {[...TECH_STACK, ...TECH_STACK].map((t, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-2 glass rounded-xl border border-[rgba(99,130,195,0.12)] flex-shrink-0">
                <span className="text-sm">{t.icon}</span>
                <span className="text-xs sm:text-sm font-medium text-[#a8c3ff]">{t.name}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {languages.map((lang, i) => (
                ...
                >
                  <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: langColor(lang) }} />
